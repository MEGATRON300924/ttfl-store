...
export type ApiCategoryVariationOption = { id: string; label: string; value: string };
export type ApiCategoryVariation = { id: string; key: string; name: string; type: CategoryVariationType; options: ApiCategoryVariationOption[] };
export type ApiCategoryVariationConfig = { enabled: boolean; variations: ApiCategoryVariation[] };
...
export type ApiCategory = { id: string; name: string; slug: string; icon: string | null; variationConfig?: ApiCategoryVariationConfig; children?: ApiCategory[] };
...
export type ApiVendorBalance = { grossSales: number; totalCommission: number; totalEarnings: number; paidOut: number; availableBalance: number; payoutAccountConfigured: boolean; payoutAccountVerified: boolean; payoutBankName: string | null; payoutAccountLast4: string | null; payoutAccountName: string | null; settlementPending: number };
export type ApiPaystackAccount = { configured: boolean; subaccountCode: string | null; bankName: string | null; accountLast4: string | null; accountName: string | null; active: boolean; verified: boolean; settlementSchedule?: string };
...
export type ApiVendorOrder = { id: string; status: OrderStatus; subtotal: string; commissionRate: string; commissionAmount: string; vendorEarnings: string; estimatedDeliveryAt?: string | null; items: ApiOrderItem[]; vendor?: { id: string; storeName: string; storeSlug: string; verified: boolean }; order?: ApiOrder };
export type ApiOrder = { id: string; orderNumber: string; totalAmount: string; currency: string; paymentStatus: PaymentStatus; paidAt: string | null; deliveryName: string; deliveryPhone?: string; deliveryLine1?: string; deliveryLine2?: string | null; deliveryCity?: string; deliveryState?: string; deliveryCountry?: string; createdAt: string; vendorOrders: ApiVendorOrder[] };
