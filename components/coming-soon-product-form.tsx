...
export function ComingSoonProductForm() {
  ...
  const [form, setForm] = useState({ name: "", description: "", categorySlug: "", availableAt: "", launchPrice: "", previousPrice: "", launchStock: "1", estimatedDeliveryDays: "7", sellingMethod: "CHECKOUT" as "CHECKOUT" | "EXTERNAL_LINK" | "WHATSAPP", externalUrl: "", whatsappNumber: "", location: "", images: [] as UploadedImage[], videos: [] as UploadedVideo[], specifications: {} as Record<string, string> });
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(null); if (form.name.trim().length < 3) return setError("Product name must be at least 3 characters."); if (form.description.trim().length < 10) return setError("Description must be at least 10 characters."); if (!form.categorySlug) return setError("Please choose a product category."); if (!form.images.length) return setError("Add at least one product image."); if (form.videos.length > 3) return setError("You can add up to 3 product videos."); if (form.availableAt && (!form.launchPrice || Number(form.launchPrice) <= 0)) return setError("Add the launch price when you set a launch date."); if (form.previousPrice && Number(form.previousPrice) <= Number(form.launchPrice)) return setError("Previous price must be greater than the launch price."); if (form.availableAt && form.sellingMethod === "EXTERNAL_LINK" && !form.externalUrl) return setError("Add the purchase URL for launch."); if (form.availableAt && form.sellingMethod === "WHATSAPP" && !form.whatsappNumber) return setError("Add the WhatsApp number for launch."); setSubmitting(true);
    try {
      ...
      if (form.availableAt) { await api.post("/api/launch-campaigns", { productId: response.product.id, launchAt: new Date(form.availableAt).toISOString(), launchDetails: { price: Number(form.launchPrice), previousPrice: form.previousPrice ? Number(form.previousPrice) : null, stock: Number(form.launchStock), estimatedDeliveryDays: Number(form.estimatedDeliveryDays), sellingMethod: form.sellingMethod, ...(form.sellingMethod === "EXTERNAL_LINK" ? { externalUrl: form.externalUrl } : {}), ...(form.sellingMethod === "WHATSAPP" ? { whatsappNumber: form.whatsappNumber } : {}) } }); }
  ...
  return <form onSubmit={handleSubmit} className="flex flex-col gap-5">
    <div className="overflow-hidden rounded-card border border-graphite-200 bg-white dark:border-graphite-700 dark:bg-graphite-900">
      ...
      <div className="flex flex-col gap-4 p-5">
        ...
        <div className="rounded-card border border-ember-200 bg-ember-50/70 p-4 dark:border-ember-900/60 dark:bg-ember-950/20"><p className="text-sm font-bold text-graphite-900 dark:text-white">🚀 Launch details</p><p className="mt-1 text-xs text-graphite-600 dark:text-graphite-400">Set the date and the details that should automatically become live when the countdown reaches zero.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><TextField label="Launch date & time" type="datetime-local" value={form.availableAt} onChange={(value) => set("availableAt", value)} hint="When this time arrives, the product becomes a normal product automatically." /><TextField label="Launch price" type="number" value={form.launchPrice} onChange={(value) => set("launchPrice", value)} optional hint="The price customers will see at launch." /><TextField label="Previous price" type="number" value={form.previousPrice} onChange={(value) => set("previousPrice", value)} optional /><TextField label="Launch stock" type="number" value={form.launchStock} onChange={(value) => set("launchStock", value)} /><TextField label="Delivery days" type="number" value={form.estimatedDeliveryDays} onChange={(value) => set("estimatedDeliveryDays", value)} /><label className="flex flex-col gap-1 text-sm"><span className="font-medium text-graphite-700 dark:text-graphite-300">How will customers buy it?</span><select value={form.sellingMethod} onChange={(e) => set("sellingMethod", e.target.value as typeof form.sellingMethod)} className="rounded-[7px] border border-graphite-200 bg-white px-3 py-2.5 text-sm dark:border-graphite-700 dark:bg-graphite-900 dark:text-white"><option value="CHECKOUT">TTFL Store checkout</option><option value="WHATSAPP">WhatsApp</option><option value="EXTERNAL_LINK">External website</option></select></label>{form.sellingMethod === "WHATSAPP" && <TextField label="WhatsApp number" value={form.whatsappNumber} onChange={(value) => set("whatsappNumber", value)} hint="Use the number customers should contact at launch." />}{form.sellingMethod === "EXTERNAL_LINK" && <TextField label="Purchase URL" value={form.externalUrl} onChange={(value) => set("externalUrl", value)} hint="Where customers should buy it at launch." />}</div></div><div className="grid gap-4 sm:grid-cols-2"><TextField label="Location" value={form.location} onChange={(value) => set("location", value)} optional hint="e.g. Lagos, Abuja, Port Harcourt." /></div>
