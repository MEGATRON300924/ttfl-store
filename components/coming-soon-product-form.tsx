...
export function ComingSoonProductForm() {
  ...
  const [form, setForm] = useState({ name: "", description: "", categorySlug: "", availableAt: "", location: "", images: [] as UploadedImage[] });
  ...
  useEffect(() => {
    let mounted = true;
    api.get<{ categories: ApiCategory[] }>("/api/categories").then((response) => {
      if (mounted) setCategories(response.categories);
    }).catch((err) => {
      if (mounted) setError(err instanceof ApiError ? err.message : "Unable to load categories.");
    }).finally(() => {
  ...
  function addTag() {
    const incoming = tagInput.split(",").map((tag) => tag.trim().toLowerCase()).filter(Boolean);
    if (!incoming.length) return;
    setTags((current) => Array.from(new Set([...current, ...incoming])).slice(0, 20));
    setTagInput("");
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    if (form.name.trim().length < 3) return setError("Product name must be at least 3 characters.");
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Something went wrong while creating the Coming Soon product.");
      setSubmitting(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="overflow-hidden rounded-card border border-graphite-200 bg-white dark:border-graphite-700 dark:bg-graphite-900">
        <div className="border-b border-graphite-200 bg-gradient-to-br from-ember-50 via-white to-gold-50 p-5 dark:border-graphite-700 dark:from-graphite-900 dark:via-graphite-900 dark:to-graphite-800">
          <div className="flex items-center gap-3">
            <div className="grid h-11 w-11 place-items-center rounded-2xl bg-graphite-900 text-xl text-white">✦</div>
            <div><p className="text-sm font-bold text-graphite-900 dark:text-white">Build anticipation</p><p className="text-xs text-graphite-600 dark:text-graphite-400">This product will appear as Coming Soon — not for sale yet.</p></div>
          </div>
        </div>
