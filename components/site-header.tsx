    ...
    <header className="sticky top-0 z-40 border-b border-graphite-200 bg-white shadow-sm dark:border-graphite-700 dark:bg-graphite-950">
      <div className="hidden border-b border-graphite-200 bg-graphite-950 text-graphite-200 md:block">
        <div className="shell flex h-9 items-center justify-between text-[13px]">
          <div className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /><span>Deliver to Nigeria</span></div>
      ...
      <div className="shell flex h-14 items-center gap-2 sm:h-16 sm:gap-4">
        <button className="grid h-9 w-9 shrink-0 place-items-center rounded-card text-graphite-700 dark:text-graphite-200 md:hidden" onClick={() => setMenuOpen(true)} aria-label="Open menu">
          <Menu className="h-5 w-5" />
        </button>

        <Logo />

        <div className="hidden flex-1 items-center md:flex"><SearchAutocomplete /></div>
      ...
      <div className="shell pb-2.5 sm:pb-3 md:hidden"><SearchAutocomplete mobile /></div>

      <nav className="hidden border-t border-graphite-200 dark:border-graphite-800 md:block">
        <div className="shell flex h-11 items-center gap-6 overflow-x-auto text-[13px] font-medium text-graphite-700 dark:text-graphite-300">
          <Link href="/services" className="shrink-0 font-semibold text-ember-600 hover:text-ember-700">Services</Link>
          {categories.map((c) => <Link key={c.id} href={`/categories/${c.slug}`} className="shrink-0 hover:text-ember-600">{c.name}</Link>)}
        </div>
      </nav>

      {menuOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          <button className="absolute inset-0 bg-graphite-950/50" aria-label="Close menu" onClick={() => setMenuOpen(false)} />
          <div className="absolute inset-y-0 left-0 flex w-[82%] max-w-xs flex-col bg-white p-5 dark:bg-graphite-950">
            <div className="mb-6 flex items-center justify-between">
              <Logo />
              <button onClick={() => setMenuOpen(false)} aria-label="Close menu"><X className="h-5 w-5 text-graphite-700 dark:text-graphite-200" /></button>
            </div>
            <div className="flex flex-col gap-1 text-[15px] font-medium text-graphite-900 dark:text-graphite-100">
              <Link href="/services" className="flex items-center gap-2 rounded-card px-2 py-2.5 font-semibold text-ember-600 hover:bg-cloud-100" onClick={() => setMenuOpen(false)}><Wrench className="h-4 w-4" />Services</Link>
