...
import { ArrowRight, BadgeCheck, Megaphone, Package, Wrench } from "lucide-react";
import { api } from "@/lib/api-client";

type Campaign = {
  id: string;
  target_type: "PRODUCT" | "SERVICE" | "COMING_SOON";
  storeName: string;
  storeSlug: string;
...
export function SponsoredPlacementCard({ campaign }: { campaign: Campaign }) {
  ...
  const title = isService ? campaign.serviceTitle : campaign.productName;
  const href = slug ? (isService ? `/services/${slug}` : `/products/${slug}`) : `/store/${campaign.storeSlug}`;
    ...
    >
      <span className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-card bg-cloud-100 dark:bg-graphite-800">
        {campaign.storeLogoUrl ? (
          <img src={campaign.storeLogoUrl} alt="" className="h-full w-full object-cover" />
        ) : (
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5">
          <span className="truncate font-semibold">{title || campaign.storeName}</span>
        </span>
        <span className="mt-0.5 flex items-center gap-1 text-xs text-graphite-500">
          {campaign.storeName}
          {campaign.storeVerified && <BadgeCheck className="h-3.5 w-3.5 text-verified-600" />}
        </span>
        <span className="mt-1 flex items-center gap-1 text-[11px] font-bold uppercase tracking-wide text-ember-600">
