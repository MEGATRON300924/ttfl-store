...
export const metadata: Metadata = {
  title: "Contact delivery driver | TTFL Store",
  description: "Securely contact the delivery driver assigned to your TTFL Store order.",
};
...
export default async function DriverContactPage({ params }: { params: { token: string } }) {
  ...
  if (!contact) {
    return <main className="shell py-16"><div className="mx-auto max-w-xl rounded-card border border-graphite-200 bg-white p-6 text-center"><h1 className="text-2xl font-bold text-graphite-900">Driver contact unavailable</h1><p className="mt-2 text-sm leading-6 text-graphite-600">This secure link may have expired, the driver may no longer be assigned, or a driver phone number has not been added yet.</p><Link href="/orders/track" className="mt-5 inline-flex rounded-card bg-ember-600 px-5 py-3 text-sm font-semibold text-white">Track my order</Link></div></main>;
  }
