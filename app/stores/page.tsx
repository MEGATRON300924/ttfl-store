  ...
  return (
    <main className="shell py-7 sm:py-10">
      <div className="mb-7 flex items-start justify-between gap-4">
        <div>
          <Link href="/" className="mb-4 inline-flex items-center gap-1.5 text-sm font-medium text-graphite-600 hover:text-ember-600">
            <ArrowLeft className="h-4 w-4" /> Back home
          </Link>
          <div className="flex items-center gap-3">
            <span className="grid h-11 w-11 place-items-center rounded-full bg-ember-100 text-ember-600"><Store className="h-5 w-5" /></span>
            <div>
      ...
      {failed ? (
        <div className="rounded-card border border-dashed border-graphite-200 p-10 text-center">
          <p className="font-semibold text-graphite-900">We couldn’t load the stores right now.</p>
          <p className="mt-1 text-sm text-graphite-600">Please refresh and try again.</p>
        </div>
      ) : stores.length === 0 ? (
        <div className="rounded-card border border-dashed border-graphite-200 p-10 text-center">
