...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  if (!product) {
    return {
      ...
      openGraph: {
        title: "Product not found | TTFL Store",
        description: "This product could not be found on TTFL Store.",
        images: [{ url: DEFAULT_SEO_IMAGE, width: 1200, height: 630, alt: "TTFL Store" }],
      },
...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  ...
  const jsonLd = {
    ...
    sku: product.publicProductId || product.id,
      ...
      <div className="shell py-6">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-4">
              <DeliveryEstimate days={product.estimatedDeliveryDays ?? 7} />
            </div>
                ...
                <dl className="divide-y divide-graphite-200 rounded-card border border-graphite-200">
                  {Object.entries(product.specifications as Record<string, string>).map(([key, value]) => (
                    <div key={key} className="flex justify-between gap-4 px-3 py-2 text-sm">
                      <dt className="text-graphite-600">{key}</dt>
                      <dd className="text-right font-medium text-graphite-900">{value}</dd>
                    </div>
                  ))}
                </dl>
              </div>
