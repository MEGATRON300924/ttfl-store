...
export default function AdminSettingsPage() {
  ...
  async function save(key: string, value: string) {
    setSavingKey(key);
    try { await api.put(`/api/settings/${key}`, { value }); await load(); } finally { setSavingKey(null); }
  }
  ...
  async function saveWhatsAppNumbers() {
    ...
    try {
      const cleaned = whatsappNumbers.split(/[,\n]/).map((number) => number.trim()).filter(Boolean).join(",");
    ...
    } catch { setWhatsappMessage("Could not save the WhatsApp admin numbers."); } finally { setSavingKey(null); }
  }
  ...
  return <div className="shell max-w-3xl py-8">
    <div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-card bg-cloud-100 text-graphite-700"><Settings className="h-5 w-5" /></span><div><h1 className="text-xl font-bold text-graphite-900">Platform settings</h1><p className="mt-1 text-sm text-graphite-600">Manage marketplace pricing, payout thresholds and WhatsApp notifications.</p></div></div>
    <section className="mt-6 rounded-card border border-graphite-200 bg-white p-5">
      <div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-card bg-cloud-100 text-graphite-700"><MessageCircle className="h-5 w-5" /></span><div><h2 className="font-bold text-graphite-900">WhatsApp admin numbers</h2><p className="mt-1 text-sm leading-6 text-graphite-600">These numbers receive TTFL Store admin WhatsApp notifications and WhatsApp test messages.</p></div></div>
      ...
      <p className="mt-2 text-xs leading-5 text-graphite-500">Use international format without spaces. Add multiple numbers on separate lines or separated by commas.</p>
      <div className="mt-4 flex items-center gap-3"><button onClick={() => void saveWhatsAppNumbers()} disabled={savingKey === "whatsapp_admin_numbers"} className="inline-flex items-center gap-2 rounded-card bg-graphite-900 px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-60"><Save className="h-4 w-4" />{savingKey === "whatsapp_admin_numbers" ? "Saving…" : "Save WhatsApp numbers"}</button>{whatsappMessage && <p className="text-xs text-verified-700">{whatsappMessage}</p>}</div>
    </section>
    <div className="mt-6 flex flex-col gap-4">{Object.entries(settings).filter(([key]) => key !== "whatsapp_admin_numbers").map(([key, value]) => <SettingRow key={key} settingKey={key} label={LABELS[key] ?? key} value={value} saving={savingKey === key} onSave={(v) => void save(key, v)} />)}</div>
  </div>;
}
