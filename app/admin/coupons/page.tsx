...
export default function AdminCouponsPage() {
  ...
  async function load() {
    const { coupons } = await api.get<{ coupons: ApiCoupon[] }>("/api/coupons/admin");
    setCoupons(coupons);
  }
  ...
  async function create(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await api.post("/api/coupons/admin", { code: form.code, type: form.type, value: Number(form.value) });
  ...
  async function toggleActive(coupon: ApiCoupon) {
    await api.patch(`/api/coupons/admin/${coupon.id}`, { active: !coupon.active });
    await load();
  }
  ...
  return (
    <div className="shell max-w-2xl py-8">
      ...
      <p className="mt-1 text-sm text-graphite-600">TTFL-wide discount codes (platform-funded, not vendor-specific).</p>
            ...
            <div key={c.id} className="flex items-center justify-between rounded-card border border-graphite-200 p-3">
              ...
              <button
                ...
                className={`rounded-tag px-2 py-1 text-xs font-medium ${
                  c.active ? "bg-verified-100 text-verified-700" : "bg-cloud-100 text-graphite-500"
                }`}
              >
