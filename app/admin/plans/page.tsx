...
export default function AdminVendorPlansPage() {
  ...
  return <div className="shell py-8"><h1 className="text-xl font-bold text-graphite-900">Vendor plans</h1><p className="mt-1 text-sm text-graphite-600">Pricing, limits, commission rates, descriptions, and public storefront capabilities per tier.</p>{plans === null ? <p className="mt-6 text-sm text-graphite-600">Loading…</p> : <div className="mt-6 grid gap-4 sm:grid-cols-2">{TIERS.map((tier) => <PlanEditor key={tier} tier={tier} plan={plans.find((p) => p.tier === tier)} />)}</div>}</div>;
}
