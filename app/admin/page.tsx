...
import { Users, Package, BarChart3, Wallet, Ticket, Megaphone, Layers, MessageCircle, Settings, FileText, Mail, ShoppingBag, Send, UserPlus, MessageSquare, UserMinus, BellRing,CalendarDays,Gift } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  return <div className="shell py-8">
    ...
    <section className="mt-6 rounded-card border border-graphite-200 bg-white p-5"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-card bg-cloud-100 text-graphite-700"><Gift className="h-5 w-5"/></span><div><h2 className="font-bold text-graphite-900">Partner event-plan access</h2><p className="mt-0.5 text-sm text-graphite-600">Give strategic partners complimentary access to the Event Plan.</p></div></div>{eventAdminMessage&&<p className="mt-3 rounded-card bg-verified-100 px-3 py-2 text-sm text-verified-700">{eventAdminMessage}</p>}{eventAdminError&&<p className="mt-3 rounded-card bg-ember-100 px-3 py-2 text-sm text-ember-700">{eventAdminError}</p>}<div className="mt-4 divide-y divide-graphite-200 border-t border-graphite-200">{partners.length===0?<p className="py-4 text-sm text-graphite-600">No partner applications yet.</p>:partners.map((partner)=><div key={partner.id} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-graphite-900">{partner.organizationName}</p><p className="text-xs text-graphite-600">{partner.owner?.email} · {partner.status} · {partner.eventPlan} plan</p></div><button type="button" onClick={()=>void grantFreeEventAccess(partner)} disabled={grantingPartnerId===partner.id} className="inline-flex items-center justify-center gap-2 rounded-card bg-graphite-900 px-3 py-2 text-xs font-bold text-white disabled:opacity-60"><Gift className="h-3.5 w-3.5"/>{grantingPartnerId===partner.id?"Granting...":partner.complimentaryAccess?"Complimentary access active":"Grant free Event Plan"}</button></div>)}</div></section>
