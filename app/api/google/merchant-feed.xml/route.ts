...
function primaryImage(product: Product): string | undefined {
  return [...product.images]
    .sort((a, b) => Number(Boolean(b.isPrimary)) - Number(Boolean(a.isPrimary)) || (a.position ?? 0) - (b.position ?? 0))[0]
    ?.url;
}
...
async function getAllProducts(): Promise<Product[]> {
  ...
  for (let page = 1; page <= 100; page += 1) {
    const response = await fetch(`${API_URL}/api/products?limit=48&page=${page}&sort=newest`, {
...
export async function GET() {
  try {
    ...
    const eligible = products.filter((product) =>
      ...
      !product.comingSoon &&
      product.sellingMethod === "CHECKOUT" &&
      Number(product.price) > 0 &&
      Boolean(primaryImage(product)) &&
    ...
    const items = eligible.map((product) => {
      ...
      const description = text(product.description, 5000);
    ...
    <item>
      ...
      <g:identifier_exists>${brand || gtin || mpn ? "yes" : "no"}</g:identifier_exists>${
        brand ? `\n      <g:brand>${xml(text(brand, 70))}</g:brand>` : ""
      }${gtin ? `\n      <g:gtin>${xml(text(gtin, 70))}</g:gtin>` : ""}${
        mpn ? `\n      <g:mpn>${xml(text(mpn, 70))}</g:mpn>` : ""
