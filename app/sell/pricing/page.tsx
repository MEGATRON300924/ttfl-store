    ...
    <div className="min-h-screen bg-cloud-50">
      <section className="border-b border-graphite-200 bg-white">
        <div className="shell py-12 text-center sm:py-16">
          <div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-ember-100 text-ember-600"><Store className="h-6 w-6" /></div>
          <p className="mt-4 text-xs font-semibold uppercase tracking-[0.16em] text-ember-600">Sell on TTFL Store</p>
          <h1 className="mt-2 text-3xl font-bold tracking-tight text-graphite-900 sm:text-4xl">Choose your vendor plan</h1>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-graphite-600">Pick the plan that fits your store. Commission is charged on successful marketplace checkout sales.</p>
        </div>
      </section>
      ...
      <div className="shell py-8 sm:py-12">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {plans.map((plan) => {
            ...
            return (
              <article key={plan.tier} className={`relative flex flex-col rounded-card border bg-white p-5 ${featured ? "border-ember-600 shadow-card" : "border-graphite-200"}`}>
        ...
        <div className="mx-auto mt-8 max-w-3xl rounded-card border border-graphite-200 bg-white p-5 text-sm leading-6 text-graphite-600">
          <strong className="text-graphite-900">How vendor payments work:</strong> customer checkout payments are split through Paystack. TTFL receives the commission for the vendor's plan and the vendor's share is settled to the vendor's connected Paystack subaccount.
        </div>
      </div>
    </div>
  );
