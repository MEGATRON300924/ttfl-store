...
async function getVendor(slug: string): Promise<PublicVendor | null> {
  try {
    ...
    try {
      ...
      const vendor = response.store ?? response.vendor ?? response;
      ...
      return {
        ...
        badges: vendor.verified ? ["VERIFIED"] : [],
        gallery: [],
      };
    } catch (fallbackError) {
    ...
    <div className={dark ? "min-h-screen bg-graphite-950" : "min-h-screen bg-cloud-50"}>
      <div className="shell py-5 sm:py-8">
        <div className={`overflow-hidden rounded-card border border-graphite-200 ${surface}`}>
          {vendor.bannerUrl ? (
            <div className="relative h-36 w-full sm:h-56"><Image src={vendor.bannerUrl} alt="" fill sizes="100vw" className="object-cover" priority /></div>
          ) : (
            <div className="h-24 w-full sm:h-32" style={vendor.theme === "MINIMAL" ? undefined : { backgroundColor: vendor.accentColor }} />
          )}

          <div className="relative px-4 pb-5 sm:px-8 sm:pb-7">
            <div className="-mt-9 flex flex-col gap-4 sm:-mt-12 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end">
                {vendor.logoUrl ? (
                  <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-full border-4 border-white bg-white shadow-card sm:h-24 sm:w-24"><Image src={vendor.logoUrl} alt={`${vendor.storeName} logo`} fill sizes="96px" className="object-cover" /></div>
                ) : (
                  <div className="grid h-20 w-20 shrink-0 place-items-center rounded-full border-4 border-white bg-graphite-900 text-2xl font-bold text-white shadow-card sm:h-24 sm:w-24">{vendor.storeName.charAt(0).toUpperCase()}</div>
                )}
                <div className="min-w-0">
                  <h1 className="truncate text-2xl font-bold sm:text-3xl">{vendor.storeName}</h1>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-[1fr_auto]">
              <div>{vendor.headline && <p className="text-lg font-semibold">{vendor.headline}</p>}<p className={`mt-1 max-w-3xl text-sm leading-6 ${muted}`}>{vendor.description ?? vendor.bio ?? "Welcome to our TTFL Store."}</p></div>
              <div className={`flex gap-5 text-sm ${muted}`}><span><strong className={dark ? "text-white" : "text-graphite-900"}>{vendor.productCount}</strong> products</span><span><strong className={dark ? "text-white" : "text-graphite-900"}>{vendor.viewCount}</strong> visits</span></div>
            </div>
          </div>
        </div>
