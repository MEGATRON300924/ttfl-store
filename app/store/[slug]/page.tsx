...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  if (!vendor) {
    return {
      ...
      description: "This store could not be found on TTFL Store.",
      openGraph: { title: "Store not found | TTFL Store", description: "This store could not be found on TTFL Store.", images: [{ url: DEFAULT_SEO_IMAGE, width: 1200, height: 630, alt: "TTFL Store" }] },
  ...
  return {
    ...
    openGraph: { type: "website", siteName: "TTFL Store", title, description, url: canonical, images: [{ url: image, width: 1200, height: 630, alt: `${vendor.storeName} on TTFL Store` }] },
          ...
          <div className="relative px-4 pb-5 sm:px-8 sm:pb-7">
            <div className="-mt-9 flex flex-col gap-4 sm:-mt-12 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end">
                ...
                <div className="min-w-0"><h1 className="truncate text-2xl font-bold sm:text-3xl">{vendor.storeName}</h1><div className="mt-2"><StoreBadges badges={badges} /></div>{vendor.location && <p className={`mt-2 flex items-center gap-1.5 text-sm ${muted}`}><MapPin className="h-4 w-4" />{vendor.location}</p>}</div>
              </div>
