...
export default function VendorDashboardPage() {
  ...
  useEffect(() => {
    ...
    void api.get<{ membership: Membership }>("/api/vendor-staff/me").then(({ membership }) => setMembership(membership)).catch(() => setMembership(null)).finally(() => setMembershipLoading(false));
  }, [loading, user]);
  ...
  const canSee = (link: (typeof LINKS)[number]) => membership?.isOwner || (link.ownerOnly ? false : membership?.permissions.includes(link.permission ?? ""));
  const visibleLinks = LINKS.filter(canSee);

  return (
    <div className="shell py-8">
      <h1 className="text-xl font-bold text-graphite-900">{user.vendorProfile?.storeName ?? "Vendor dashboard"}</h1>
      {!membership?.isOwner && membership && <p className="mt-1 text-sm text-graphite-600">Team role: <span className="font-semibold">{membership.role}</span></p>}
      {user.vendorProfile && user.vendorProfile.status !== "APPROVED" && <div className="mt-4 flex items-start gap-3 rounded-card bg-gold-100 p-4"><AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-gold-600" /><div><p className="text-sm font-semibold text-graphite-900">{user.vendorProfile.status === "PENDING" && "Your application is under review"}{user.vendorProfile.status === "REJECTED" && "Your application was not approved"}{user.vendorProfile.status === "SUSPENDED" && "Your store is currently suspended"}</p><p className="mt-0.5 text-sm text-graphite-700">{user.vendorProfile.status === "PENDING" && "You can't list products yet — we'll email you once you're approved."}{user.vendorProfile.status === "REJECTED" && "Contact support if you'd like to appeal this decision."}{user.vendorProfile.status === "SUSPENDED" && "Contact support to resolve this."}</p></div></div>}
