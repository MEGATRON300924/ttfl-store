...
import { Package, ShoppingBag, Plus, AlertCircle, CreditCard, Wallet, BarChart3, Megaphone, Ticket, Settings, Palette, Users, Zap, Bell, Rocket, TrendingUp, PartyPopper, Landmark, AlertTriangle, Trash2, Power } from "lucide-react";
...
export default function VendorDashboardPage() {
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([
      ...
      const orders = ordersResult.status === "fulfilled" ? ordersResult.value.vendorOrders : [];
  ...
  useEffect(() => {
    if (!growth) return;
    const timer = window.setInterval(() => setTipIndex((current) => current + 1), 5000);
  ...
  async function handleAccountAction(action: "disable" | "delete") {
    const message = action === "delete"
      ...
      : "Disable your TTFL Store account? You will be logged out and won't be able to sign in until the account is reactivated.";
  ...
  const tips = growth ? [
    { icon: TrendingUp, title: nearLimit ? "You're close to your product limit" : "Increase your business on TTFL Store", body: nearLimit ? `You have ${Math.max(0, growth.productLimit! - growth.productCount)} product slot${growth.productLimit! - growth.productCount === 1 ? "" : "s"} left on your ${growth.tier} plan.` : "Upgrade your plan to list more products and unlock a lower marketplace commission.", href: "/vendor/dashboard/subscription", action: "View plans" },
    { icon: Landmark, title: "Add your payout details", body: "Add your bank details so your TTFL Store earnings can be paid to you when they become available.", href: "/vendor/dashboard/payouts", action: "Add bank details" },
  ] : [];
  const activeTip = tips.length ? tips[tipIndex % tips.length] : null;
  return <div className="shell py-8">
    {activeTip && <div className="mb-5 overflow-hidden rounded-card border border-ember-200 bg-ember-50 dark:border-ember-500/30 dark:bg-ember-950/30"><div key={tipIndex % tips.length} className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-card bg-ember-600 text-white"><activeTip.icon className="h-5 w-5" /></span><div><p className="text-sm font-bold text-graphite-900 dark:text-white">{activeTip.title}</p><p className="mt-0.5 text-sm text-graphite-600 dark:text-graphite-300">{activeTip.body}</p></div></div><Link href={activeTip.href} className="inline-flex shrink-0 items-center justify-center rounded-card bg-ember-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-ember-700">{activeTip.action}</Link></div><div className="flex gap-1 px-4 pb-3" aria-hidden="true">{tips.map((_, index) => <span key={index} className={`h-1 flex-1 rounded-full ${index === tipIndex % tips.length ? "bg-ember-600" : "bg-ember-200 dark:bg-ember-800"}`} />)}</div></div>}
