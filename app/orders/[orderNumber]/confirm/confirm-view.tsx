...
export function OrderConfirmView() {
  ...
  useEffect(() => {
    ...
    api
      .get<{ order: ApiOrder }>(`/api/orders/verify/${reference}`)
      .then(({ order }) => {
        setOrder(order);
    ...
    <div className="shell py-16 text-center">
      ...
      {status === "success" && (
        <>
          ...
          <p className="mt-1 text-sm text-graphite-600">
            Order <span className="font-mono">{params.orderNumber}</span> has been paid and sent to the vendor(s).
          </p>
          <Link
            href="/account"
      ...
      {status === "failed" && (
        <>
          ...
          <p className="mt-1 text-sm text-graphite-600">
            If you were charged, contact support with order {params.orderNumber} — nothing was lost from your cart.
          </p>
          <Link
            href="/support"
